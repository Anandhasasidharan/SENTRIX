"""Narrated-completion detection for plan output (the banking_3 gap).
...
passes. This module detects that failure mode.
...
* So this check is purpose-built for the plan-output distribution:
  ...
  action/completion language is matched against the set of tools that
  were ACTUALLY executed, and only actions whose verb-family has no
  backing executed tool are flagged. A plan that narrates "List the
  files" and really executed ``list_dir`` does not fire.
...
2. Imperative actions ("Execute the Update", "Send the email", "Query
   the payment system"): a finding unless an executed tool covers the
   verb's family (exec/delete/send/write/read).
3. Completion claims ("I have updated", "changes saved", "was
   executed"): a finding unless the family is covered.
4. Commitment language ("I will proceed with executing the plan"): a
   finding only when nothing was executed at all — a plan that both
   commits and executes is internally consistent.
"""
...
def _family_covered(family: str, executed_tools: list[str]) -> bool:
...
def detect_narrated_actions(
    plan_text: str, executed_tools: list[str] | None = None
) -> list[dict]:
    """Return narrated-action findings, or [] if the plan is clean/refusal."""
    ...
    for m in IMPERATIVE_PATTERN.finditer(text):
        ...
        if family and not _family_covered(family, executed_tools):
            findings.append(
                {
                    "kind": "imperative",
                    "phrase": m.group(0),
    ...
    for m in COMPLETION_PATTERN.finditer(text):
        ...
        for verb, family in _COMPLETION_VERB_FAMILY.items():
            if matched.endswith(verb):
                if not _family_covered(family, executed_tools):
                    findings.append(
    ...
    if COMMITMENT_PATTERN.search(text) and not executed_tools:
        findings.append(
            {
                "kind": "commitment",
